...
st.markdown("""
## そもそもNISAって何？
通常、投資の利益には約20%の税金がかかるけど、NISAを使えば利益がすべて非課税になる国のお得なボーナス制度！
""")
...
# グラフ描画用にデータフレームを作成（ラベルをわかりやすく変更）
df = pd.DataFrame({
    "貯金だけの場合（投資元本）": principal_list,
    "NISAで運用した場合": total_list
...
# ★ 貯金との比較をわかりやすく表示
st.markdown("### 📊 20年後の結果発表")
st.error(f"🏦 **貯金だけの場合**：約 **{int(final_principal):,}** 円 （増えたお金：0円）")
st.success(f"🎉 **NISAで運用した場合**：約 **{int(final_total):,}** 円 （増えたお金：約 **{int(profit):,}** 円！）")
st.info(f"💡 NISAなら、本来引かれるはずの税金 約 **{int(tax_saved):,}** 円がタダになる！")
...
**💡 暴落したときの心構え**  
ニュースなどで株価が大きく下がったと聞いても、絶対に焦って売らないこと！ 途中でやめず、長期間コツコツ続ける「長期・分散・積立」が投資を成功させる一番のコツです。
""")

# 2つの枠の違い
st.markdown("""
### 📝 NISAの2つの枠の違い
NISAには2つの枠があり、これらは同時に使う（併用する）こともできる！
...
st.markdown("""
### 🛒 何を買えばいいの？（代表的な2つ）
初心者のうちは、「つみたて投資枠」で以下のどちらかの投資信託を毎月買う設定にするのが定番！
...
st.markdown("""
### 🏢 どこで始めるのがおすすめ？
NISAの口座を作るなら、以下のネット証券が圧倒的におすすめ
* **楽天証券** または **SBI証券**
